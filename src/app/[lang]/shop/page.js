...
export default function Home({ params: { lang }}) {
	...
	const [data, setData] = useState({ products: null, filter: 'view_all' });
	...
	useEffect(() => {
		...
		async function getProducts() {
			...
			setCategories(['view_all', ...res_categories.map((category) => category.title).reverse()]);
	...
	const productsFilter = data.filter == 'view_all' ? data.products : data.products.filter(product => product.category.title == data.filter);
